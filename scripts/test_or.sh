...
MODEL_DIR="models/or"
RESULTS_DIR="results/or"
MODEL_FILE="$MODEL_DIR/or.smt2"
TIMEOUT=30000

mkdir -p "$RESULTS_DIR"
rm -f "$RESULTS_DIR"/*.txt  # Очистка

echo "Анализ модели OR: $MODEL_FILE" > "$RESULTS_DIR/results.txt"
...
test_variant "no_zero_exclude" "sed '/(assert (or (> x1_low 0) .+ ))/d' '$MODEL_FILE'"
