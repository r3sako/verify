...
MODEL_DIR="models/sub"
RESULTS_DIR="results/sub"
MODEL_FILE="$MODEL_DIR/sub.smt2"
TIMEOUT=30000

mkdir -p "$RESULTS_DIR"
rm -f "$RESULTS_DIR"/*.txt  # Очистка

echo "Анализ модели вычитания: $MODEL_FILE" > "$RESULTS_DIR/results.txt"
...
# Ослабления
test_variant "no_borrow" "sed '/(assert (or (= lowBorrow[12] 0) (= lowBorrow[12] 1)))/d' '$MODEL_FILE' | sed '/(assert (or (= highBorrow[12] 0) (= highBorrow[12] 1)))/d'"
...
test_variant "no_mod_two32" "sed '/(assert (= (mod (- a[12] b[12]) two32) .+ ))/d' '$MODEL_FILE'"
...
test_boundary "overflow_borrow" 0 0 1 0   # low = -1 → borrow=1
test_boundary "high_overflow" 0 65535 0 1 # high = -1 → borrow=1
