...
(declare-const and1_low Int)  ; x & y для low и high одинаково
...
;; BitwiseXor = x + y - 2*(x & y) — точно как в u32.zir
(assert (= and1_low  (mod (* x1_low y1_low) p)))
(assert (= and1_high (mod (* x1_high y1_high) p)))
(assert (= z1_low  (mod (+ x1_low y1_low (* -2 and1_low)) p)))
(assert (= z1_high (mod (+ x1_high y1_high (* -2 and1_high)) p)))

(assert (= and2_low  (mod (* x2_low y2_low) p)))
(assert (= and2_high (mod (* x2_high y2_high) p)))
(assert (= z2_low  (mod (+ x2_low y2_low (* -2 and2_low)) p)))
(assert (= z2_high (mod (+ x2_high y2_high (* -2 and2_high)) p)))

;; ищем различие — должно быть unsat
